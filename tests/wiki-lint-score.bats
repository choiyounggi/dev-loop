...
@test "Health score section states total_weight=40, weights 3/2/1, and the health: NN/100 format" {
  section="$(section_body "$SKILL" '^## Health score')"
  [[ "$section" == *"total_weight = 40"* ]] && [[ "$section" == *"| error | 3 |"* ]] && [[ "$section" == *"| warn | 2 |"* ]] && [[ "$section" == *"| info | 1 |"* ]] && [[ "$section" == *"health: NN/100 (errors E, warns W, infos I)"* ]]
}

# --- negative control: the pin discriminates on the total_weight constant ---

@test "negative control: a SKILL.md copy with total_weight reverted to 39 fails the constant check" {
  sed 's/total_weight = 40/total_weight = 39/' "$SKILL" > "$BATS_TEST_TMPDIR/skill.md"
  section="$(section_body "$BATS_TEST_TMPDIR/skill.md" '^## Health score')"
  [[ "$section" != *"total_weight = 40"* ]] && [[ "$section" == *"total_weight = 39"* ]]
}
