...
deciding who may build or refresh the graph (an agent versus a user-installed
git hook).
...
| An agent is about to build or refresh the graph — a coordinator mid-run, or a SessionStart hook that found a graph stale or absent | Report the state (stale N, absent) and hand the choice to the human, e.g. via a nudge to the onboarding skill; the build runs only on that explicit choice |
| A git hook the user installed once with explicit consent (post-commit, post-checkout, or dev-loop's post-merge) runs graphify update after a commit, checkout, or pull | Allowed — git is the actor and the consent was given at onboarding; the hook exits 0 when graphify is absent so it never blocks the user's own git operation |
...
| The retrieval layer is a vector index over a fixed-format corpus (dev-loop's bundled wiki) rather than a code graph | Expose it as an MCP with the tool count bounded to two — one search that returns page ids and snippets, one read by id — because the cost directive 5 guards against accrues per tool definition, and keep the CLI form (`scripts/wiki-index.py search`) for hooks and lint |
...
- https://www.anthropic.com/engineering/code-execution-with-mcp — most MCP clients load every tool definition into context up front; the two-tool wiki server (dev-loop issue #202) is the bounded exception recorded in Edge cases
- Local source reading 2026-09-06 (graphifyy 0.4.23: `watch.py` `_rebuild_code`, `export.py` `MAX_NODES_FOR_VIZ = 5_000`, `__main__.py` `update`): the rebuild writes the report and `graph.json`, then `to_html` raises `ValueError("... too large for HTML viz. Use --no-viz ...")`; the `except` returns `False` and `update` exits 1; no `--no-viz` option is parsed anywhere in the CLI. Field reproduction 2026-09-04 (a 9,663-node repository): the failed `update` had already refreshed `graph.json`, and with `to_html` wrapped the same rebuild completed in about 3 s. PyPI listed graphifyy 0.9.54 as latest on 2026-09-06, so re-check `graphify update --help` on the installed version before relying on this row
