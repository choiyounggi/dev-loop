...
sources:
  ...
  - https://github.com/anthropics/claude-code/issues/36320
...
2. **Read a synchronized multi-worker stop as one shared allowance, not N
   ...
   parallel workers draw one pool and reach the wall together. The docs name this
   case directly: "a single burst of heavy activity, such as a large workflow
   fanout, can exhaust the weekly allowance before the session window resets."
...
| Automatic resume is expected from the CLI | It is not implemented — resuming after a reset is a standing feature request, so the orchestrator owns the re-drive |
...
- https://code.claude.com/docs/en/costs — "each member's Claude Code usage draws from a per-seat allowance that resets on a rolling five-hour window and a weekly window. The allowance is shared with Claude chat and Cowork"; "a single burst of heavy activity, such as a large workflow fanout, can exhaust the weekly allowance before the session window resets"; agent teams spawn multiple Claude Code instances whose usage scales with the number of active teammates
...
- https://github.com/anthropics/claude-code/issues/36320 — automatic resume when the usage window resets is an open feature request, so the caller owns the re-drive after a limit pause
