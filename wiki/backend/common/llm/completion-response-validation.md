...
| DeepSeek first-party API (not an OpenAI-style gateway) | Its chain-of-thought has a separate budget and does not consume `max_tokens`, so the blank-content-from-reasoning mode does not arise there; keep the `finish_reason` and blank-`content` checks anyway |
...
- Field context: 2026-07-28 LiteLLM (internal) measurement — a reasoning-routed alias returned HTTP 200, `finish_reason=length`, 0-char `content`, and an 8,173-char `reasoning_content`; the empty artifact flowed to document registration until this gate was added
