...
| [zero-exit-diagnostics-into-a-hook](shells/zero-exit-diagnostics-into-a-hook.md) | Wiring a compiler/linter/validator into an agent loop, PostToolUse hook, CI wrapper, or watch script so its warnings reach the model; the tool prints diagnostics to stderr but exits 0 and an exit-code branch never fires; choosing the redirection order that captures stderr without the build artifact; choosing which exit code and stream actually deliver text to the model |
| [unset-versus-empty-parameters](shells/unset-versus-empty-parameters.md) | Disabling a script's optional behavior by setting its environment variable to the empty string and the feature keeps running; writing a knob whose empty value must mean "off"; reviewing a script that reads its options with `${VAR:-default}`; distinguishing unset from empty from set-to-a-value in a shell script |
