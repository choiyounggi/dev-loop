...
| [escapes-in-shell-string-literals](shells/escapes-in-shell-string-literals.md) | Writing a regex, glob, `sed`/`awk` program, `jq` filter, or `printf` format as a shell string literal (e.g. a hardcoded `grep -E` pattern in a hook) and a metacharacter behaves as though an escape were added or removed; an end-of-line `\$` anchor matches or fails unexpectedly; deciding whether to single- or double-quote a pattern literal; a `grep -F` check for a markdown literal containing a backtick span reports no match while the text is in the file |
