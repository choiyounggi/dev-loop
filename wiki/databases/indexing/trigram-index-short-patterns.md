...
| Short keywords must return results and the operator is `LIKE` | Evaluate `pg_bigm`, which "allows a user to create 2-gram (bigram) index" and whose own comparison rates 1–2 character keyword search "Fast" against pg_trgm's "slow" |
...
| The workload is non-alphabetic text (Japanese, Chinese, Korean) | pg_trgm's own comparison lists full text search for non-alphabetic language as "Not supported", so the 3-character rule bites ordinary two-character words — treat pg_bigm as the default candidate rather than the fallback |
