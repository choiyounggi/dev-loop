...
| Three or more successive fixes have failed, each revealing new coupling somewhere else | Stop generating fix-level hypotheses — the failing hypothesis is the design itself. Escalate to an architecture-level review of the component's boundaries before attempting a fourth fix; a pattern of fixes that each surface new breakage elsewhere is evidence of a structural cause no local patch reaches |
| The failure spans several components and no single suspect stands out | Instrument every component boundary in one pass — log the data entering and leaving each seam — run the reproduction once, and read which layer the data goes wrong in; then form hypotheses inside that one layer only |
...
- https://github.com/obra/superpowers — systematic-debugging skill: the 3-failed-fixes architecture circuit breaker and instrument-every-boundary-then-run-once for multi-component failures; field-tested across agentic debugging sessions
