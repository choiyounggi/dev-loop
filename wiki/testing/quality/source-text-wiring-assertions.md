...
2. **Make the assertion's subject the file with comments removed, taken from a
   tokenizer.** Comments carry the same tokens the guard searches for — a JSDoc
   naming the component, a `// guard: never use X here` note — and text search
   cannot tell prose from code; ESLint states the split, "While comments are not
   technically part of the AST". From a test, call an API that actually yields
   them: `espree.parse(src, {comment: true, range: true}).comments`, `@babel/parser`
   `parse(src).comments`, TS `ts.getLeadingCommentRanges`, Python
   `tokenize.COMMENT`. Measured 2026-08-10: `ts.createSourceFile(...)` leaves
   `sourceFile.comments` undefined and emits no comment nodes, so it strips nothing
   and the guard silently keeps its raw subject. Fixture-test the stripper: comment
   text gone, every string and regex literal byte-identical afterwards.
...
4. **Set the bound N from the distance the anchor and call have in the stripped
   subject from step 2 — the same text the assertion runs against — plus the
   length of one statement**, so a legitimately inserted
...
6. **Run a semantics-preserving control and require green** — change a comment or
   ...
   ([testing-quality-harness-reverse-controls]). Run the halves separately: with
   step 2 in place a red comment-only edit indicts the stripper, while a red
   reformat means N from step 4 is too small.
...
| Strip comments with `src.replace(/\/\*[\s\S]*?\*\//g,'').replace(/\/\/.*$/gm,'')`          | Take the comment ranges from the parser (step 2), and fixture-test the stripper                   | Measured 2026-08-10 in Node: that pair truncates `"https://api.example.com//v2/items"` to `"https:` — the `//` inside a string literal is read as a comment start, so from that point on the guard searches a truncated subject and its verdict no longer describes the file |
...
- Measurement 2026-08-10 (Node + TypeScript 5, fixture with a JSDoc block, a line comment, a trailing comment, a URL string literal and a regex literal): on the raw source, `<BlockDetailPanel` matched 2 times where the code has 1 site, and `/Number\.isFinite/` tested true from a `// guard: never use Number.isFinite here` comment. After comment removal both returned the code's own values (1, false) — removal by the regex pair below, which on this fixture selects the same spans a parser would; the parser is what step 2 requires for arbitrary input. The regex stripper in the same run corrupted a string literal (`"https://…//v2/items"` → `"https:`), and a string-aware variant still corrupted a regex literal (`/a//b/` → `/a`) — the basis for the two `Instead of` rows. Same session: `ts.createSourceFile('a.ts', src, ts.ScriptTarget.Latest, true)` returned `comments === undefined` with AST kinds `SourceFile,FunctionDeclaration,…,EndOfFileToken` and no comment node, while `ts.getLeadingCommentRanges(src, 0)` returned the range — the basis for step 2 naming the range/scanner APIs. A reformat control reddened a `{0,20}`-bounded pattern with no comment involved, and a trailing comment left a line-filtering stripper's count at 2 against 1 for cut-to-EOL
- Field measurement 2026-08-10 (rtb-unified, `feat-NEWRTB-2451-detail-entry-rule`): three source-text guards in one PR reddened on correct code, each from a comment — a JSDoc mentioning `<BlockDetailPanel` (mount-count assertion), a comment naming `Number.isFinite` (negative assertion), and a comment containing `SOURCE.slice(...)` (a self-check forbidding a raw slice). All three were resolved by asserting on comment-stripped code, and a comment-only control edit stayed green afterwards. Field measurement 2026-08-24 (same repo, PR #896) reproduced both directions twice in one session: a shell comment quoting the superseded `${identifier//[[:space:]]/}` reddened a `not.toContain` (false red), and a `globalSetup` comment containing `[expected-identity source:` was selected by `indexOf` ahead of the code, collapsing the guarded region to `''` (vacuous green — it surfaced only because the ordering happened to redden). After the comment-stripping filter was added, seven seeded mutations were killed by their intended assertions and a no-op control survived
