...
| [polling-completion-predicates](quality/polling-completion-predicates.md) | Writing or reviewing a monitor, watcher, or wait loop that decides "all work is finished" by matching text in a status file or command output, and that verdict gates a downstream action (merge, report, next stage); such a monitor announced completion far too early; choosing between a count-based and an absence-based done condition; matching a status token that contains regex metacharacters (`[queued]`, `a.b`) through nested shell/ssh/tmux quoting; deciding what a wait loop does when its deadline expires |
