...
related: [testing-strategy-test-level-choice, testing-quality-behavior-not-implementation, testing-quality-minimum-case-set, qa-process-regression-scope]
---
...
- https://arxiv.org/pdf/2102.07498 — differential testing "has been widely used for checking the consistency of two (or more) alternative implementations of a common specification": generate inputs, run each through both implementations, and check the outputs are equivalent
- https://handwiki.org/wiki/Differential_testing — differential testing (also called back-to-back testing) uses different implementations of the same functionality as cross-referencing oracles: any discrepancy on the same input is a potential bug, a stronger oracle than fuzzing's crash detection
- https://arxiv.org/pdf/2212.01748 — experience report applying differential testing to compiler-optimization verification, where the reference implementation supplies the oracle a single implementation lacks
- Field observation 2026-08-05: a platform with an interpreter path and a compiled-binary path fixed four classes as required-to-agree (execution order, policy outcome, observability signals, masking) and five as permitted-to-differ (scheduler shape, memory placement, instruction selection, operation count, wall-clock time); its check compares only the former, on the stated grounds that comparing timing would fail for reasons the contract allows
